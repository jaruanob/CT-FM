...
def process_single_scan_pair(scan_pair: Tuple[Dict[str, str], Dict[str, str]], patch_size: Tuple[int, int, int] = (64, 64, 64), model_type=Model_type.ct_fm) -> float:
    """
    ...
    try:
        ...
        # Perform search
        match_point, _ = search(query_scan["image"], target_scan["image"], query_point, patch_size, model_type=model_type)
        
        # Calculate distance
        distance = torch.dist(torch.tensor(match_point, dtype=torch.float32), query_point.float()) / 3
        return distance.item() / 10  # Convert to mm
...
def process_batch_scans_parallel(scan_pairs: List[Tuple[Dict[str, str], Dict[str, str]]], num_processes: int = None, model_type=Model_type.ct_fm) -> List[float]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default="/mnt/data1/TotalSegmentator/v2/processed", help='Directory containing the data')
    parser.add_argument('--model_type', type=str, default="ct-fm", help='Model type to use for processing')
    ...
    distances = process_batch_scans_parallel(scan_pairs, num_processes=args.num_processes, model_type=model_type)
